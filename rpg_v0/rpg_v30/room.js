...
function collect_root() {
    
    for(i=0; i < cha_location.root.length;i++){
        console.log(this.id);console.log(cha_location.root[i].name);
        if (this.id == cha_location.root[i].name){
            elf.inventory.push(cha_location.root[i]);
            cha_location.root.splice(i, 1); break;           
        }
    }
    load_root();console.log(elf.inventory);
